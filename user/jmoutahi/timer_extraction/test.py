...
df = (
    ...
    .apply(lambda x: x.sort_values("number"))
...
df["time_seconds_rolling"] = (
    df.groupby("mat_number")["time_seconds"]
...
df["time_diff_rolling_small"] = df.groupby("mat_number")[
    "time_seconds_rolling_small"
].diff()
...
rolling_ones = (
    df.groupby("mat_number")["timer_paused"]
...
for mat_number in df["mat_number"].unique():
    ...
    plt.plot(
        df[df["mat_number"] == mat_number]["time_seconds"],
    ...
    ax2 = plt.gca().twinx()
    ax2.plot(
        df[df["mat_number"] == mat_number]["time_diff_rolling_small"],
        label="Time difference",
        color="r",
    )
    # ax2.set_ylabel("Time difference")
    ax2.plot(
        df[df["mat_number"] == mat_number]["time_diff_rolling"],
        label="Time difference (rolling mean)",
        color="y",
    )

    # Make the plot background green match and red non-match
    plt.fill_between(
        df[df["mat_number"] == mat_number].index,
        0,
        100,
        where=df[df["mat_number"] == mat_number]["match"] == 1,
        color="g",
        alpha=0.3,
    )
    plt.fill_between(
        ...
        0,
        100,
        where=df[df["mat_number"] == mat_number]["match"] == 0,
        color="r",
        alpha=0.3,
    )
    ...
    plt.fill_between(
        ...
        0,
        100,
        where=df[df["mat_number"] == mat_number]["timer_paused"] == 1,
        color="y",
        alpha=0.6,
    )

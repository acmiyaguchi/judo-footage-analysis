...
def data_on_video(video_path, json_path, output_path="output.mp4", N=None):
    ...
    print(f"frame_width: {frame_width}, frame_height: {frame_height}, fps: {fps}, number_of_frames: {number_of_frames}, number_of_frames in df: {len(df)}")
    ...
    for index, row in df.iterrows():
        ...
        text = [
            ...
            "Not processed : " + "Paused" if row['time_seconds_derivative'] == 0 else "Running",
            "Processed : " + "Paused" if row['time_seconds_derivative_over150frames'] == 0 else "Running",
        ]

        for j, t in enumerate(text):
            cv2.putText(frame, t, (10, 30 * (j + 1)), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)
...
def data_as_subtitles(video_path, json_path, output_path="output.srt", N=None):
    ...
    with open(output_path, "w") as f:
        ...
        for index, row in df.iterrows():
            ...
            f.write(f"{i+1}\n")
            time = index // fps
            time = pd.to_datetime(time, unit="s").strftime('%H:%M:%S,000') # 00:00:00,000
            # Write the time
            f.write(f"{time} --> ")
            time = (index) // fps + 1
            ...
            text = [
                ...
                f"Not processed timer : {row['time_seconds']}",
                f"Processed timer : {row['time_seconds_filled']}",
                "Not processed : Paused" if row['time_seconds_derivative'] == 0 else "Not processed : Running",
                "Processed : Paused" if row['time_seconds_derivative_over150frames'] == 0 else "Processed : Running",
            ]

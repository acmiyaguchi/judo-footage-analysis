...
def extract_points_from_folder(folder_path, json_path, rois):
    ...
    for filename in os.listdir(folder_path):
            if filename.endswith(('.jpg', '.jpeg', '.png', '.bmp')):
                ...
                img[roi_coordinates_to_hide[1]:roi_coordinates_to_hide[3], roi_coordinates_to_hide[0]:roi_coordinates_to_hide[2]] = 0

                # Invert the colors for the region player one
                img[roi_coordinates_player_one[1]:roi_coordinates_player_one[3], roi_coordinates_player_one[0]:roi_coordinates_player_one[2]] = cv2.bitwise_not(img[roi_coordinates_player_one[1]:roi_coordinates_player_one[3], roi_coordinates_player_one[0]:roi_coordinates_player_one[2]])

                # Extract the specified region of interest
                img_roi = img[roi_coordinates_full_band[1]:roi_coordinates_full_band[3], roi_coordinates_full_band[0]:roi_coordinates_full_band[2]]
...
def extract_timer_from_video(video_path, json_path, roi_coordinates):
    ...
    for i in range(0, total_frames, batch_size):
        ...
        for j in range(i, min(i + batch_size, total_frames)):
            ...
            if ret:
                # Extract the specified region of interest
                roi = frame[roi_coordinates[1]:roi_coordinates[3], roi_coordinates[0]:roi_coordinates[2]]
                ...
                results.append({'frame_number': j, 'available': result, 'minutes': minutes, 'seconds': seconds, 'raw_text': text, 'filename': video_path})
...
def parallelize_extraction_points(frame_folders, json_folder, rois):
    with ThreadPoolExecutor(max_workers=None) as executor:
        futures = []
        for frame_folder in frame_folders:
            futures.append(executor.submit(extract_points_from_folder, frame_folder, json_folder, rois))
...
def parallelize_extraction_timer_video(video_folders, json_folder, roi_coordinates):
    with ThreadPoolExecutor(max_workers=None) as executor:
        ...
        for video_folder in video_folders:
            ...
            futures.append(executor.submit(extract_timer_from_video, video_folder, json_folder, roi_coordinates))

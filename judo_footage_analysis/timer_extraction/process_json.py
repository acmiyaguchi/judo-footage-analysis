...
def process_json(timer_json="combined_videos.json", filename="timer_filled.json", frame_rate=30, verbose=False):
    ...
    if(verbose) : print(df.head(1))

    # For each mat, sort the frames by the frame number in ascending order
    df = df.groupby("mat_number").apply(lambda x: x.sort_values("frame_number", ascending=True)).reset_index(drop=True)
    ...
    window = frame_rate
    df["time_seconds_filled"] = df["time_seconds_filled"].rolling(window, center=True).mean()
    if(verbose) : print(df.head())
    ...
    df["time_seconds_derivative"] = df["time_seconds_derivative"].apply(lambda x: -1 if x < 0 else 0)
    if(verbose) : print(df.head())
...
def plot_timer(timer_json="timer_filled.json", frame_rate=30):
    ...
    for mat_number, mat_df in df.groupby("mat_number"):
        ...
        ax[0].plot(mat_df["time_seconds"].values, "g*", label="Original timer extracted using Tesseract")
        ...
        ax[1].plot(mat_df["time_seconds_derivative"].values, "b-", label="Derivative of the timer in seconds")
        for N in list_N:
            ax[1].plot(mat_df["time_seconds_derivative_over" + str(N) + "frames"].values, label=f"Derivative of the timer in seconds over {N} frames")
